...
class Chat extends MY_Controller {
    ...
    function index() {
        ...
        $fetchchat = $this->mdl_chat->fetchchat($username, $recipient);
        $chatreverse = '';
    ...
    function get_new_messages() {
        ...
        //get unread messages
        $result = $this->mdl_chat->get_new_messages($this->session->userdata('username'));
        $ctr = 0;
        $users = array();
        ...
        foreach ($users as $online_users) {
            ...
            $check_if_online = $this->mdl_chat->check_if_online($online_users);
    ...
    function sendmessage() {
        ...
        $this->mdl_chat->sendmessage($data);
    }
    ...
    function chatbuddy() {
        ...
        $data['username'] = $this->session->userdata('username');
        $data['recipient'] = $this->session->userdata('recipient');
        $this->mdl_chat->message_read($data);
    }
/*
 * close_buddy
 *
